...
export async function generateMarkdownReport(
    ...
    // Try LLM-based report for higher quality, fall back to deterministic on error
    try {
        const sourcesList = Array.from(allSources.values()).map((source, idx) => {
            const title = source.title || 'Unknown Source';
            const url = source.url || '';
            const snippet = source.snippet
                ? (source.snippet.length > 300 ? source.snippet.substring(0, 300) + '...' : source.snippet)
                : '';
            return `${idx + 1}. ${title}\n${url}${includeFullSources && snippet ? `\nSnippet: ${snippet}` : ''}`;
        }).join('\n\n');

        const qaList = queryAnswerPairs.map((pair, idx) => {
            const answer = pair.answer || 'No detailed findings for this query.';
            return `Q${idx + 1}: ${pair.query}\nA${idx + 1}: ${answer}`;
        }).join('\n\n');

        const system = `You are a research report writer. Produce a clear, well-structured markdown report.
Rules:
- Use citations like [1], [2] that map to the numbered sources list.
- Do not invent sources or facts beyond the provided content.
- Prefer concise, high-signal writing.
- Output only markdown.`;

        const user = `Title: ${firstQuery}
Generated: ${generatedAt}
Topics: ${queryAnswerPairs.length}
...
Include a "Sources" section listing all sources with title and URL. ${includeFullSources ? 'Include snippets under each source.' : ''}
${includeDiagrams ? 'Include a small Mermaid flowchart under a "Research Process" section.' : 'Skip diagrams.'}

Required sections:
1) Executive Summary
2) Key Findings (bulleted)
3) Detailed Analysis (subsections per query)
4) Sources
5) Further Questions (optional if relevant)
`;

        const llmReport = await LLMService.generateResponse([
            { id: 'report_sys', conversationId: 'report', role: 'system', content: system, timestamp: Date.now() },
            { id: 'report_user', conversationId: 'report', role: 'user', content: user, timestamp: Date.now() },
        ] as any, { tools: [] });

        const cleaned = (llmReport || '').trim();
        if (cleaned) {
            return cleaned;
        }
    } catch (e) {
        console.warn('[ReportService] LLM report failed, falling back:', e);
    }
